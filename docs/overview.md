...
| KG-2.11 | **Research Intelligence Pipeline** | `agent-utilities` (inherited) |
| KG-2.12 | **KG Source Resolver** | `agent-utilities` (inherited) |
...
```json
{
  "mcpServers": {
    "scholarx": {
      "command": "uv",
      "args": ["run", "--with", "scholarx", "scholarx-mcp"],
      "env": {}
    }
  }
}
...
```bash
scholarx-mcp --transport streamable-http --port 8001
```
