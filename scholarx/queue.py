...
pool) rather than by serializing a flat sleep per worker. (CONCEPT:SX-1.6)
"""
...
# Number of worker threads draining JOB_QUEUE in parallel. (CONCEPT:SX-1.6)
POOL_SIZE = 4
...
# worker serializing on a flat per-job sleep. (CONCEPT:SX-1.6)
...
def _rate_gate() -> None:
    ...
    job start across the pool, then claim this start slot. (CONCEPT:SX-1.6)"""
...
def _download_worker():
    ...
    Multiple instances run concurrently (one per pool thread). (CONCEPT:SX-1.6)
    """
