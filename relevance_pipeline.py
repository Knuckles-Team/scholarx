...
async def main():
    ...
    print("=" * 70)
    print("ScholarX — 30-Paper Relevance-Scored Pipeline")
    print("=" * 70)
    ...
    print("\n🔍 Fetching 30 papers from arXiv cs.AI...")
    result = await client.search(query)
    print(f"   Retrieved: {result.total_count} papers")
...
"""
    ...
    print(f"📄 Saved {len(accepted)} paper markdowns to {OUTPUT_DIR}")
    ...
    print(f"\n📥 Downloading PDFs for {len(accepted)} accepted papers...")
    print(f"   Rate limit: {DOWNLOAD_DELAY}s between downloads (arXiv policy)")
    downloaded = 0
    failed = []
    for i, sp in enumerate(accepted, 1):
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    ...
                    print(f"  ❌ [{i}/{len(accepted)}] Failed after {MAX_RETRIES} attempts: {e}")
                    failed.append(sp["paper"].title)

    if failed:
        print(f"\n  ⚠️  {len(failed)} downloads failed after retries:")
        for t in failed:
            print(f"     - {t[:70]}")

    print(f"\n{'=' * 70}")
    print("✅ Pipeline complete!")
    print(f"   Papers fetched:  {len(scored_papers)}")
